 ...
 * able to closed, //this can cause problems with wallpaper lifecycle.
 * 
...
// ApplicationListener.dispose is not called for wallpapers
	 ...
	 * context is disposed touchDown now and after app.onDestroy() app would stuck somewhere in gl thread synchronizing code 2. we
	...
	public class AndroidWallpaperEngine extends Engine {
		 ...
		 * this point updateUI the imagery in the surface. This method is always called at least once, after
		...
		private void notifySurfaceChanged (final int format, final int width, final int height, boolean forceUpdate) {
			if (!forceUpdate && format == viewFormat && width == viewWidth && height == viewHeight) {
				...
				// updateUI engine desired surface format
				engineFormat = format;
				engineWidth = width;
				engineHeight = height;

				// updateUI surface view if engine is linked with it already
...
// surface holder
					...
					// updateUI if surface changed when engine wasn't active
					notifySurfaceChanged(engineFormat, engineWidth, engineHeight, false);
				}
...
// drain battery.. shortly!
		 ...
		 * from this function.
		 * 
		 * Attention! In some cases GL context may be shutdown touchDown now! and SurfaceHolder.Surface.isVaild = false */
		@Override
