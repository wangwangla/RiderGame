 ...
 * @author Nathan Sweet
 */
public class Group extends Actor implements Cullable,Cloneable {
    static protected final Vector2 tmp = new Vector2();
...
//        Actor[] actors = children.begin();
//        for (int i = 0, n = children.size; i < n; i++){
//            if(actors[i] == null) children.removeValue(actors[i],false);
//            actors[i].act(delta);
//        }
//        children.end();
//    }
 ...
 //           if (actor.parent == this) return;
    ...
    public <T extends Actor> Array<T> findActorAll(String name) {
        Array<Actor> children = this.children;
        Array<T> all = new Array<>();
        for (int i = 0, n = children.size; i < n; i++)
            if (name.equals(children.get(i).getName())) all.add((T)children.get(i));
        for (int i = 0, n = children.size; i < n; i++) {
            Actor child = children.get(i);
            if (child instanceof Group) {
                Actor actor = ((Group) child).findActor(name);
                if (actor != null) all.add((T)children.get(i));
            }
        }
        return all;
    }

    public <T extends Actor> T findActor(String name,Touchable touchable) {
        Array<Actor> children = this.children;
        for (int i = 0, n = children.size; i < n; i++)
