		...
		if (parameter != null && parameter.atlasName != null) {
			if (parameter.isAtlas){
				...
				if (parameter.preFix == null) {
					...
					region = atlas.findRegion(parameter.preFix +"/"+name);
				}
				if (region == null)
					throw new GdxRuntimeException("Could not find font region " + name + " in atlas " + parameter.atlasName);
				return new BitmapFont(file, region);
			}else {
				PlistAtlas atlas = manager.get(parameter.atlasName, PlistAtlas.class);
				String name = file.sibling(data.imagePaths[0]).nameWithoutExtension().toString();
				AtlasRegion region;
				if (parameter.preFix == null) {
					region = atlas.findRegion(name);
